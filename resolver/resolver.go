...
// It takes the scanner.ProjectInfo, converts deps to app.Dependency,
// runs the PubGrub solver against PyPI, then rewrites the manifest.
func Run(info *scanner.ProjectInfo, verbose bool) RunResult {
	fmt.Println()
	fmt.Println("──────────────────────────────────────────────────────")
	fmt.Println("[Phase 2] Resolving versions via PubGrub + PyPI...")
...
func rewritePackageJSON(projectPath string, pinned map[string]string) error {
	...
	// read existing package.json to preserve name/version/scripts
	existing := map[string]interface{}{}
	if data, err := os.ReadFile(pkgPath); err == nil {
		json.Unmarshal(data, &existing)
	}

	// replace dependencies with pinned versions
